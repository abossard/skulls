...
It also treats agent-authored knowledge as claims rather than facts. A modern agent wiki starts as an evidence-backed context map: linked Markdown, repository search, one owner per piece of detail, and pointers from claims to the observations or primary sources that support them.
...
- **Evidence-backed context maps**: every durable claim records its status (sourced, observed, inferred, or unknown), evidence, scope, checked date, and invalidation condition. `CONTEXT.md` holds vocabulary, ADRs hold decisions, work items hold work, evidence artifacts hold observations, and `CONTEXT-MAP.md` only indexes them.
...
Ask which load you want to pay and which artifact owns the claim. `CLAUDE.md` loads into every [session](https://www.aihero.dev/ai-coding-dictionary/session) unconditionally; material behind a pointer costs only the pointer's own line until it fires. Vocabulary belongs in `CONTEXT.md`, decisions in ADRs, active work in issues or specs, and observations beside their tests, CI runs, browser captures, prototypes, or primary sources. The always-loaded file should point, not copy.
...
- A durable factual claim tells you whether it is sourced, observed, inferred, or unknown, and points to evidence with a scope, date, and invalidation condition.
- The context map stays a small index; loading it does not load the specs, decisions, logs, and research it points at.

## Where it fits

This is a reach-for-it-anytime standalone reference. It has no neighbour in the chain because it sits underneath the whole set rather than beside any one skill: every skill here was written against it, and the documents the other skills leave behind (a `CONTEXT.md` and its ADRs, a spec, a ticket) are exactly the text it governs once an agent has to read them. When you're unsure which skill or flow fits a task, [ask-andre](https://github.com/abossard/skulls/blob/main/docs/engineering/ask-andre.md) routes you over the whole set.
